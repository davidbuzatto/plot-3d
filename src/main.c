...
static Mesh createSurfaceModel( float xMin, float xMax, float yMin, float yMax, int steps ) {
...
static void init( ) {
    ...
    Mesh mesh = createSurfaceModel( X_MIN, X_MAX, Y_MIN, Y_MAX, STEPS );
    surfaceModel = LoadModelFromMesh( mesh );
    surfaceModel.materials[0].maps[MATERIAL_MAP_DIFFUSE].color = BLUE;
...
static void draw( void ) {
    ...
    DrawModel( surfaceModel, (Vector3) {0}, 1.0f, WHITE );
    DrawModelWires( surfaceModel, (Vector3) {0}, 1.0f, BLACK );
